...
IMAGE_NAME="ghcr.io/notifiarr/starrproxy"
TAG="local"
...
generate_changelog() {
    ...
    COMMITS=$(git log "$FROM_TAG..HEAD" --pretty=format:"- [\`%h\`](https://github.com/notifiarr/starrproxy/commit/%H): %s by @%an" --reverse 2>/dev/null)
...
**Full Changelog**: https://github.com/notifiarr/starrproxy/compare/$FROM_TAG...$1
EOF
}
