...
if ($_POST['m'] == 'deleteStarr') {
    unset($settings[$app][$_POST['instance']]);
...
if ($_POST['m'] == 'saveStarr') {
    ...
    if ($_POST['instance'] == '99') {
        $settings[$app][] = ['name' => $name, 'url' => $_POST['url'], 'apikey' => $_POST['apikey']];
    } else {
        if ($_POST['instance'] >= 0) {
            $settings[$app][$_POST['instance']] = ['name' => $name, 'url' => $_POST['url'], 'apikey' => $_POST['apikey']];
        }
    }
...
if ($_POST['m'] == 'newAppStarrAccess') {
    $existing       = $_POST['id'] != 99 ? $settings['access'][$app][$_POST['id']] : [];
    $clone          = isset($_POST['clone']) ? $settings['access'][$app][$_POST['clone']] : [];
    ...
    if ($settings[$app]) {
        foreach ($settings[$app] as $instance => $instanceSettings) {
        ...
        <tr>
            ...
            <td>
                ...
                foreach ($endpoints as $endpoint => $endpointInfo) {
                    ...
                    ?><h5 class="d-inline-block me-2"><?= $endpointInfo['label'] ?></h5><span class="text-small"><?= $endpoint ?></span><br><?php
                    ...
                    foreach ($endpointInfo['methods'] as $method) {
                        ...
                        ?><input <?= $checked ?> id="endpoint-counter-<?= $counter ?>" data-endpoint="<?= $endpoint ?>" data-method="<?= $method ?>" type="checkbox" class="form-check-input endpoint-<?= $method ?>"> <?= $method ?><br><?php
                        $counter++;
                    }
...
if ($_POST['m'] == 'saveAppStarrAccess') {
    ...
    if ($_POST['id'] != 99) {
        $access['usage'] = $settings['access'][$app][$_POST['id']]['usage'];
        $settings['access'][$app][$_POST['id']] = $access;
    } else {
...
if ($_POST['m'] == 'deleteAppStarrAccess') {
    unset($settings['access'][$app][$_POST['id']]);
    setFile(APP_SETTINGS_FILE, $settings);
}
...
if ($_POST['m'] == 'openAppAccessLog') {
    ...
    ?>
    Tip: <code>Ctrl + End</code> to view endpoint usage<br><br>
    <table class="table table-bordered table-hover">
    <?php
    if ($lines) {
        $proxiedApp     = getAppFromKey($_POST['key'], true);
        $endpointUsage  = [];
        ...
        ?><tr><td><h4>Endpoint usage <span class="text-small">(<?= count($endpointUsage) ?> endpoint<?= count($endpointUsage) == 1 ? '' : 's' ?>)</span></h4></td></tr><?php
            ...
            foreach ($methods as $method => $usage) {
                ...
                <tr>
                    <td>
                        ...
                        [<?= strtoupper($method) ?>] <?= $endpoint . ': ' . number_format($usage) ?> hits
                    </td>
                </tr>
                <?php
            }
...
if ($_POST['m'] == 'saveTemplateStarrAccess') {
    $existing = $settings['access'][$app][$_POST['id']]['endpoints'];
...
if ($_POST['m'] == 'viewTemplate') {
    ...
    ?>
    <h5><?= ucfirst($starr) ?> ... <?= str_replace('.json', '', $app) ?></h5>
...
if ($_POST['m'] == 'resetUsage') {
    unset($settings['access'][$app][$_POST['id']]['usage']);
    setFile(APP_SETTINGS_FILE, $settings);
}

if ($_POST['m'] == 'addEndpointAccess') {
    $settings['access'][$app][$_POST['id']]['endpoints'][$_POST['endpoint']][] = $_POST['method'];
    print_r($settings['access'][$app][$_POST['id']]);
    setFile(APP_SETTINGS_FILE, $settings);
}
